<?php
class Application_Model_ServiceGoodStockOutAcc extends Zend_Db_Table_Abstract{
	protected $_name = 'service_good_stock_out_acc';

	function ServiceProductStockOutAccListfetchPagination($page, $limit, &$total, $params){
        $db = Zend_Registry::get('db');
        $userStorage = Zend_Auth::getInstance()->getStorage()->read();

        //print_r($params);die;
        $select = $db->select();
        $select->from(array('p' => $this->_name),array(new Zend_Db_Expr('SQL_CALC_FOUND_ROWS p.id'),'p.data_date','p.good_code','p.good_replacement_code','p.channel','p.detail','p.price','p.retail_price','p.create_by','p.create_date','p.status'));
