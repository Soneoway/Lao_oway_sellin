...
class Application_Model_Market extends Zend_Db_Table_Abstract
{
    ...
    public function fetchInvoice($params)
    {
        ...
        ->group(new Zend_Db_Expr('p.sn,p.good_id,p.good_color'));
    ...
    function fetchPagination($page, $limit, &$total, $params){
        ...
        if (isset($params['group_sn']) and $params['group_sn']){
            ...
            $select_fields = array(new Zend_Db_Expr('SQL_CALC_FOUND_ROWS DISTINCT p.id'), 'sn_ref' => 'sn_ref', 'total_qty' => 'SUM(p.num)', 'total_price_amount' => 'SUM(p.total)','delivery_fee', 'total_price' => 'SUM(p.total) - ifnull(cn.use_discount,0) + ifnull(p.delivery_fee,0) ','invoice_number', 'type', 'print_picking_list', 'service','confirm_so','sale_off_percent' );
        ...
        //Tanong Check Payment Type
        $select->joinLeft(array('ck' => 'checkmoney_paymentorder'), 'ck.sn=p.sn', array('ck.payment_order'));

        $select->joinLeft(array('cn' => 'credit_note_tran'), 'cn.sales_order=p.sn', array('(cn.use_discount) AS total_discount'));
    ...
    */
    ...
    function so_stock_card($params) {
        ...
        $select = $db->select()
            ...
            ->joinLeft(array('w'  => 'warehouse')       ,'m.warehouse_id = w.id'    ,array('wh_from' => 'w.name'))
            ...
            ->joinLeft(array('gc' => 'good_category')   ,'m.cat_id = gc.id'         ,array('category' => 'gc.name'))
            ->joinLeft(array('d'  => 'distributor')     ,'m.d_id = d.id'            ,array('wh_to' => 'd.title'))
            ->where('m.isbacks = 0')
    ...
    function return_stock_card($params) {
        ...
        $select = $db->select()
            ...
            ->joinLeft(array('w'  => 'warehouse')       ,'m.warehouse_id = w.id'    ,array('wh_to' => 'w.name'))
