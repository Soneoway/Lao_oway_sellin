...
// start : confirm section 
    ...
    $view_accessories = $this->getRequest()->getParam('view_accessories',all);
    $this->view->view_accessories  = $view_accessories;
    if (isset($tmp) && $tmp != 0) {
        ...
        $this->_redirect('/sales/index-accessories');
    }
    // end : confirm section 
    //print_r($_POST);die;
    ...
    $view_accessories  = $this->getRequest()->getParam('view_accessories');
    ...
    $params = array_filter(array(
            ...
            'view_accessories'     => $view_accessories
    ...
    if (isset($export) && $export)
    {
        ...
        { //export for finance: use to import to account system
            ...
            $tmp2 = explode('WHERE', $tmp[1]);   
            $sql = $tmp[0].", ( 
                CASE 
                    WHEN p.d_id = 3691 OR NOW() >= '2016-04-18 00:00:00' THEN 
                        SUM( ROUND( ( (p.total/p.num)/1.07 ),2) * p.num )+ ROUND((p.delivery_fee/1.07),2) 
                    WHEN p.sn_ref = 'SO590407-00082' OR p.sn_ref = 'SO590408-00294' THEN
                        SUM( ROUND( ( (p.total/p.num)/1.07 ),2) * p.num )+ ROUND((p.delivery_fee/1.07),2) 
                    WHEN p.sn_ref = 'SO590405-00222' THEN 
                        SUM( ROUND( ( p.total/1.07 ),2) )+ ROUND((p.delivery_fee/1.07),2)
                    ELSE 
                        SUM( ROUND( ( p.total/1.07 ),2) )+ ROUND((p.delivery_fee/1.07),2)
                END ) as sum_total, c.customer_name  
                FROM ".$tmp2[0]." 
                LEFT JOIN `customer_brandshop` as c 
                    ON p.customer_id = c.customer_id 
                WHERE 
                ".$tmp2[1]." GROUP BY p.sn";
            ...
            */
            ...
            $params['export'] = $export;
            $markets_sn = $QMarket->fetchPagination($page, null, $total, $params);
            $tmp = explode('DISTINCT', $markets_sn);
            $sql = $tmp[0]." DISTINCT SUM(p.total) as sum_total,".$tmp[1]." GROUP BY p.sn";
            ...
            */
            ...
            $params['export'] = $export;
            $markets_sn = $QMarket->fetchPagination($page, null, $total, $params);
            $this->_exportExcelByModel($markets_sn);
            ...
            */
            ...
            $params['export'] = $export;
            $markets_sn = $QMarket->fetchPagination($page, null, $total, $params);
            $this->_exportExcelOutputVatImei($markets_sn);

            //echo($markets_sn);die;
            /*
            $tmp = explode('WHERE', str_replace("DISTINCT", " i.imei_sn, ", $markets_sn) );
            $sql = $tmp[0]." 
                LEFT JOIN `imei` AS i 
                    ON p.sn = i.sales_sn 
                    AND p.good_id <> 127 
