...
if ($sn) {
    ...
    $order_type = $market_data->type;

    $sales = $QMarket->getDetailMarketAndDistributor($sn, $order_type);
    ...
    foreach ($sales as $k => $sale) {
        ...
        //get Store
        $data[$k]['store_name'] = isset($stores[$sale['store_id']]) ? $stores[$sale['store_id']] : '';

        $data[$k]['store_code'] = isset($sale['store_code']) ? $sale['store_code'] : '';
        $data[$k]['store_tel'] = isset($sale['store_tel']) ? $sale['store_tel'] : '';
        $data[$k]['store_code'] = isset($sale['store_code']) ? $sale['store_code'] : '';
        $data[$k]['store_address'] = isset($sale['store_address']) ? $sale['store_address'] : '';
        $data[$k]['store_owner'] = isset($sale['store_owner']) ? $sale['store_owner'] : '';
        ...
        //get Brand Name
        $brand_ = $QBrand->getBrand($sale['good_id']);

        $data[$k]['brand_name'] = $brand_[0]['brand_name'];

        // print_r($brand_[0]['brand_name']);
