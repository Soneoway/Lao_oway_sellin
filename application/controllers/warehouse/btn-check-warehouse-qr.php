...
try {
    ...
    if ($this->getRequest()->getMethod() == 'POST') {
        ...
        if (!$sn) {
            echo json_encode(['status' => 404, 'message' => 'Not found SN']);
            exit();
        }
        ...
        if (!$m) {
            echo json_encode(['status' => 404, 'message' => 'Order Number not found']);
            exit();
        }
        $sales_out = $QMarket->fetchAll($where);
        $total_imei_scanned_before = $total_digital_scanned_before = $total_ilike_scanned_before = $total_imei = $total_digital = $total_ilike = 0;
        $order_has_phone = $order_has_digital = $order_has_ilike = false;
        $a_ordered_phones = $a_ordered_digital = $a_ordered_ilike = $a_submitted_phones_group = $a_submitted_digital_group = $a_submitted_ilike_group = $a_order_phones_group = $a_order_digital_group = $a_order_ilike_group = $a_imei_success = $a_digital_sn_success = $a_ilike_sn_success = $a_imei_errors = $a_digital_sn_errors = $a_ilike_sn_errors = array();
        // check đơn hàng có được confirm out chưa
        foreach ($sales_out as $item) {
            if ($item['outmysql_time']) {
                echo json_encode(['status' => 404, 'message' => 'This Order was confirmed OUT already!']);
                exit();
            }
            ...
            if ($userStorage->warehouse_type != '3') {
                if (!$item['pay_time']) {
                    echo json_encode(['status' => 404, 'message' => 'This Order was not confirmed payment yet!']);
                    exit();
                }
                if (!$item['shipping_yes_time']) {
                    echo json_encode(['status' => 404, 'message' => 'This Order was not confirmed to Warehouse proccess yet!']);
                    exit();
                }
            }
            if ($item['cat_id'] == PHONE_CAT_ID) {
                ...
                if (isset($a_order_phones_group[$item['good_id']][$item['good_color']]) and $a_order_phones_group[$item['good_id']][$item['good_color']]) $a_order_phones_group[$item['good_id']][$item['good_color']]+= $item['num'];
                else $a_order_phones_group[$item['good_id']][$item['good_color']] = $item['num'];
                ...
                if (isset($a_order_digital_group[$item['good_id']][$item['good_color']]) and $a_order_digital_group[$item['good_id']][$item['good_color']]) $a_order_digital_group[$item['good_id']][$item['good_color']]+= $item['num'];
                else $a_order_digital_group[$item['good_id']][$item['good_color']] = $item['num'];
                ...
                if (isset($a_order_ilike_group[$item['good_id']][$item['good_color']]) and $a_order_ilike_group[$item['good_id']][$item['good_color']]) $a_order_ilike_group[$item['good_id']][$item['good_color']]+= $item['num'];
                else $a_order_ilike_group[$item['good_id']][$item['good_color']] = $item['num'];
        ...
        if ($order_has_phone) {
            ...
            if ($total_imei < count($imei_list)) {
                echo json_encode(['status' => 400, 'message' => 'Redundant (' . (count($imei_list) - $total_imei) . ') IMEIs.']);
                exit();
            }
            if ($total_imei < (count($imei_list) + $total_imei_scanned_before)) {
                echo json_encode(['status' => 400, 'message' => 'Redundant (' . ((count($imei_list) + $total_imei_scanned_before) - $total_imei) . ') IMEIs.']);
                exit();
            }
            ...
            foreach ($imei_list as $imei) {
                ...
                if (isset($a_submitted_phones_group[$imei_info['good_id']][$imei_info['good_color']]) and $a_submitted_phones_group[$imei_info['good_id']][$imei_info['good_color']]) $a_submitted_phones_group[$imei_info['good_id']][$imei_info['good_color']]++;
                else $a_submitted_phones_group[$imei_info['good_id']][$imei_info['good_color']] = 1;
                for ($i = 0;$i < count($a_ordered_phones);$i++) {
                    // boc bo vao submitted
                    if ($a_ordered_phones[$i]['good_id'] == $imei_info['good_id'] and $a_ordered_phones[$i]['good_color'] == $imei_info['good_color']) { // neu dung san pham do thi bat dau
                        // neu quota con thi con nhet duoc
                        $quota = $a_ordered_phones[$i]['num'] - (count($a_ordered_phones[$i]['submitted']) + count($a_ordered_phones[$i]['scanned_before']));
                        if (!$quota) continue;
                        else {
            ...
            foreach ($a_order_phones_group as $good_id => $item) {
                foreach ($item as $good_color => $val) {
                    if (isset($a_submitted_phones_group[$good_id][$good_color])) {
                        ...
                        if ($a_submitted_phones_group[$good_id][$good_color] + $total_scanned_out_imei_before > $val) {
                            exit('<div class="alert alert-error">' . $goods_cached[$good_id] . ' / ' . $good_colors_cached[$good_color] . ' Ordered: ' . $val . ' / Scanned + Old: ' . $a_submitted_phones_group[$good_id][$good_color] . ' + ' . $total_scanned_out_imei_before . '</div>');
                            echo json_encode(['status' => 400, 'message' => $goods_cached[$good_id] . ' / ' . $good_colors_cached[$good_color] . ' Ordered: ' . $val . ' / Scanned + Old: ' . $a_submitted_phones_group[$good_id][$good_color] . ' + ' . $total_scanned_out_imei_before]);
                            exit();
                        }
                    }
                }
            ...
            if ($order_has_digital) {
                if (!$a_submitted_digital_sns) {
                    echo json_encode(['status' => 400, 'message' => 'No Digital SN Input.']);
                    exit();
                }
                ...
                if ($total_digital < count($digi_list)) {
                    echo json_encode(['status' => 400, 'message' => 'Redundant (' . (count($digi_list) - $total_digital) . ') Digital SNs.']);
                    exit();
                }
                if ($total_digital < (count($digi_list) + $total_digital_scanned_before)) {
                    echo json_encode(['status' => 400, 'message' => 'Redundant (' . ((count($digi_list) + $total_digital_scanned_before) - $total_digital) . ') Digital SNs.']);
                    exit();
                }
                if (count(array_unique($digi_list)) < count($digi_list)) {
                    echo json_encode(['status' => 400, 'message' => 'Duplicated Digital SNs']);
                    exit();
                }
                ...
                foreach ($digi_list as $digi) {
                    ...
                    if (isset($a_submitted_digital_group[$digi_info['good_id']][$digi_info['good_color']]) and $a_submitted_digital_group[$digi_info['good_id']][$digi_info['good_color']]) $a_submitted_digital_group[$digi_info['good_id']][$digi_info['good_color']]++;
                    else $a_submitted_digital_group[$digi_info['good_id']][$digi_info['good_color']] = 1;
                    for ($i = 0;$i < count($a_ordered_digital);$i++) {
                        // boc bo vao submitted
                        if ($a_ordered_digital[$i]['good_id'] == $digi_info['good_id'] and $a_ordered_digital[$i]['good_color'] == $digi_info['good_color']) { // neu dung san pham do thi bat dau
                            // neu quota con thi con nhet duoc
                            $quota = $a_ordered_digital[$i]['num'] - (count($a_ordered_digital[$i]['submitted']) + count($a_ordered_digital[$i]['scanned_before']));
                            if (!$quota) continue;
                ...
                foreach ($a_order_digital_group as $good_id => $item) {
                    foreach ($item as $good_color => $val) {
                        if (isset($a_submitted_digital_group[$good_id][$good_color])) {
                            $total_scanned_out_digital_sn_before = $QMarket->count_out_digital($sn, $good_id, $good_color);
                            // neu so cu + so nhap ma lon hon so cua don hang thi dung ngay
                            if ($a_submitted_digital_group[$good_id][$good_color] + $total_scanned_out_digital_sn_before > $val) {
                                echo json_encode(['status' => 400, 'message' => $goods_cached[$good_id] . ' / ' . $good_colors_cached[$good_color] . ' Ordered: ' . $val . ' / Scanned + Old: ' . $a_submitted_digital_group[$good_id][$good_color] . ' + ' . $total_scanned_out_digital_sn_before]);
                                exit();
                            }
                        }
                    }
            ...
            if ($order_has_ilike) {
                if (!$a_submitted_ilike_sns) {
                    echo json_encode(['status' => 400, 'message' => 'No Good SN Input.']);
                    exit();
                }
                ...
                if ($total_ilike < count($ilike_list)) {
                    echo json_encode(['status' => 400, 'message' => 'Redundant (' . (count($ilike_list) - $total_ilike) . ') Good SNs.']);
                    exit();
                }
                if ($total_ilike < (count($ilike_list) + $total_ilike_scanned_before)) {
                    echo json_encode(['status' => 400, 'message' => 'Redundant (' . ((count($ilike_list) + $total_ilike_scanned_before) - $total_ilike) . ') Good SNs.']);
                    exit();
                }
                if (count(array_unique($ilike_list)) < count($ilike_list)) {
                    echo json_encode(['status' => 400, 'message' => 'Duplicated Good SNs']);
                    exit();
                }
                ...
                foreach ($ilike_list as $ilike) {
                    ...
                    // tinh tong so digital sn da submit theo group
                    if (isset($a_submitted_ilike_group[$ilike_info['good_id']][$ilike_info['good_color']]) and $a_submitted_ilike_group[$ilike_info['good_id']][$ilike_info['good_color']]) $a_submitted_ilike_group[$ilike_info['good_id']][$ilike_info['good_color']]++;
                    else $a_submitted_ilike_group[$ilike_info['good_id']][$ilike_info['good_color']] = 1;
                    for ($i = 0;$i < count($a_ordered_ilike);$i++) {
                        // boc bo vao submitted
                        if ($a_ordered_ilike[$i]['good_id'] == $ilike_info['good_id'] and $a_ordered_ilike[$i]['good_color'] == $ilike_info['good_color']) { // neu dung san pham do thi bat dau
                            // neu quota con thi con nhet duoc
                            $quota = $a_ordered_ilike[$i]['num'] - (count($a_ordered_ilike[$i]['submitted']) + count($a_ordered_ilike[$i]['scanned_before']));
                            if (!$quota) continue;
                ...
                foreach ($a_order_ilike_group as $good_id => $item) {
                    foreach ($item as $good_color => $val) {
                        if (isset($a_submitted_ilike_group[$good_id][$good_color])) {
                            $total_scanned_out_ilike_sn_before = $QMarket->count_out_digital($sn, $good_id, $good_color);
                            // neu so cu + so nhap ma lon hon so cua don hang thi dung ngay
                            if ($a_submitted_ilike_group[$good_id][$good_color] + $total_scanned_out_ilike_sn_before > $val) {
                                echo json_encode(['status' => 400, 'message' => $goods_cached[$good_id] . ' / ' . $good_colors_cached[$good_color] . ' Ordered: ' . $val . ' / Scanned + Old: ' . $a_submitted_ilike_group[$good_id][$good_color] . ' + ' . $total_scanned_out_ilike_sn_before]);
                                exit();
                            }
                        }
                    }
