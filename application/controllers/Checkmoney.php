...
INNER JOIN 
(SELECT t.sn,m.sn_ref,s.username,t.bank,b.name AS bank_name,t.pay_money,DATE_FORMAT(t.pay_time, '%Y-%m-%d %H:%i')AS payment_date
FROM checkmoney t
...
";
    ...
    public function _getDataForExcelCashCollection($data,$params)
    {
        ...
        if($arr_d_id == 0){
            if(isset($params['action_from']) AND $params['action_from']=='checkmoney'){
                ...
                if (isset($params['created_at_from']) and $params['created_at_from']){
                    ...
                    if (isset($day) and isset($month) and isset($year) ){
                        ...
                        $where_invoice .=" AND (m.add_time >= '".$where_date."') ";
                    }
                }
                ...
                if (isset($params['invoice_time_from']) and $params['invoice_time_from']){
                    ...
                    if (isset($day) and isset($month) and isset($year) ){
                        $where_date = $year.'-'.$month.'-'.$day.' '.$from_time;
                        $where_invoice .=" AND (m.invoice_time >= '".$where_date."') ";
                    }
                }
            ...
            '1' AS `company_id`,
             (select IFNULL((SUM(cnt.use_discount)-ch.pay_money)*-1,ch.pay_money) from checkmoney ch left join credit_note_tran cnt on cnt.sales_order = ch.sn  where ch.sn = `m`.`sn` and ch.type = 2) AS `pay_money`,
             (select IFNULL((SUM(cnt.use_discount)-ch.pay_money)*-1,ch.pay_money) from checkmoney ch left join credit_note_tran cnt on cnt.sales_order = ch.sn  where ch.sn = `m`.`sn` and ch.type = 2) AS `output`,
            ...
            `m`.`payment_type`,
            '' AS file_pay_slip, 
            '1' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
          FROM
             `market` AS `m` 
          WHERE 1=1
          ...
          //  echo $sql_checkmoney_payment_surplus_this;die;
          ...
          /*--------------Tranfer payment_surplus_unuse เงินเกิน------------------*/
            ...
            ch.payment_no AS payment_no_ref ,m.bs_campaign
          FROM
          ...
          /*--------------Tranfer payment_surplus_before_surplus เงินเกิน------------------*/
            ...
            m.`canceled`,
            m.`canceled_remark`,
            SUBSTRING(m.pay_text, - 14, 14) AS sales_order_ref,ch.payment_no AS payment_no_ref ,m.bs_campaign
          FROM
            pay_group_tran pt 
          ...
          //  echo $sql_checkmoney_payment_surplus_before;die;  
            ...
            m.payment_no,
              ...
              m.`canceled_remark`,
              '' AS sales_order_ref,
              '' AS payment_no_ref ,m.bs_campaign
            FROM pay_group_bank bg
            INNER JOIN `checkmoney` `ch`  ON ch.`payment_id` = bg.`payment_id` AND bg.`status` = 1 
            ...
            m.payment_type,
            '' AS file_pay_slip,
            '3' as seq,'' as pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
          FROM
          ...
          GROUP BY ch.id,
            ...
            `m`.`payment_type`,
            '' AS file_pay_slip,
            '4' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
          FROM
             `market` AS `m` 
          WHERE 1=1
          ...
          GROUP BY ch.id,
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '6' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '7' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type` ,
            `ch`.`file_pay_slip`,
            '8' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,SUBSTRING(m.pay_text,-14,14)AS sales_order_ref,'' as payment_no_ref,m.bs_campaign
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
           ...
           // echo $sql_checkmoney_pay_banktransfer;die;
            ...
            `m`.`payment_type` ,
            `ch`.`file_pay_slip`,
            '2' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref,m.bs_campaign
            FROM
              `market` AS `m`
