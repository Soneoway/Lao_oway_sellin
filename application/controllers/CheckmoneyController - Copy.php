...
class CheckmoneyController extends My_Controller_Action{
    ...
    public function getbalanceAction(){
       ...
       // $row            = $QStoreAccount->fetchRow($where);
        ...
        $select->where('ship.status is null');
    ...
    public function listAction(){
       ...
       // $to_time               = $this->getRequest()->getParam('to_time');
        ...
        if($export=='1'){
            ...
            ini_set('memory_limit', -1);
            //$data = $QCheckmoney->fetchCashCollection(NULL, NULL, $total, $params);
            $this->_getDataForExcelDailyCashInView($data,$params);
        }else if($export=='4'){
            set_time_limit( 0 );
            error_reporting( 0 );
            ini_set('display_error', 0);
            ini_set('memory_limit', -1);
            //$data = $QCheckmoney->fetchCashCollection(NULL, NULL, $total, $params);
            $this->_getDataForExcelDailyCashInForBankCheck($data,$params);
        }
       ...
       // print_r($data);die;
        ...
        if($arr_d_id == 0){
            ...
            if( isset($params['invoice_number']) && $params['invoice_number']){
                $where_invoice=" AND (m.invoice_number = '".$params['invoice_number']."') ";
            }
            ...
            '1' AS `company_id`,
             (select IFNULL((SUM(cnt.use_discount)-ch.pay_money)*-1,ch.pay_money) from checkmoney ch left join credit_note_tran cnt on cnt.sales_order = ch.sn COLLATE utf8_unicode_ci where ch.sn = `m`.`sn` and ch.type = 2) AS `pay_money`,
             (select IFNULL((SUM(cnt.use_discount)-ch.pay_money)*-1,ch.pay_money) from checkmoney ch left join credit_note_tran cnt on cnt.sales_order = ch.sn COLLATE utf8_unicode_ci where ch.sn = `m`.`sn` and ch.type = 2) AS `output`,
            ...
            `m`.`payment_type`,
            '' AS file_pay_slip, 
            '1' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
          FROM
             `market` AS `m` 
          WHERE 1=1
            ...
            `ch`.`company_id`,
            ROUND((IF(m.pay_group =1,bg.`balance`,`ch`.`pay_money`)), 2) AS pay_money,
            ROUND((IF(m.pay_group =1,bg.`balance`,`ch`.`pay_money`)), 2) AS output,
            ...
            ''  AS `creditnote_sn`,
            ''  AS `deposit_sn`,
            IF(m.pay_group =1,b_bg.`name`,`b`.`name`) AS bank_name,
            `m`.`invoice_number`,
            `m`.`sn_ref`,
            `m`.`payment_type` ,
            `ch`.`file_pay_slip`,
            '2' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `market` AS `m` 
                ON m.sn = ch.sn 
              left join pay_group_bank bg on ch.`payment_id`=bg.`payment_id` and bg.`status`=1 
              LEFT JOIN `bank` AS `b` ON b.id = ch.bank  
              LEFT JOIN `bank` AS `b_bg` ON b_bg.id = bg.bank 
            WHERE 1=1
            AND ch.sn <>'0' 
            AND ch.type ='1'
            ...
            m.payment_type,
            '' AS file_pay_slip,
            '3' as seq,'' as pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
          FROM
          ...
          GROUP BY ch.id,
            ...
            `m`.`payment_type`,
            '' AS file_pay_slip,
            '4' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
          FROM
             `market` AS `m` 
          WHERE 1=1
            ...
            `ch`.`sn`,
            '7' as `TYPE`,m.payment_no,ifnull(ch.lack_of_money,0)as lack_of_money,ifnull(ch.payment_surplus,0)as payment_surplus,
            `ch`.`company_id`,
            ...
            `m`.`payment_type`,
            ch.file_pay_slip,
            '5' as seq ,'' as pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '6' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '7' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,'' as sales_order_ref,'' as payment_no_ref
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type` ,
            `ch`.`file_pay_slip`,
            '8' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`,SUBSTRING(m.pay_text,-14,14)AS sales_order_ref,'' as payment_no_ref
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            /*--------------Tranfer payment_surplus เงินเกิน------------------*/
            $sql_checkmoney_payment_surplus=" select t.* from( SELECT 
    `m`.`sn`,
    '11' AS `TYPE`,
    ch.payment_no,
    0 AS lack_of_money,
    ...
    m.`canceled`,
    m.`canceled_remark`,
    SUBSTRING(m.pay_text, - 14, 14) AS sales_order_ref,p.payment_no AS payment_no_ref 
  FROM
    pay_group_tran pt 
    LEFT JOIN `pay_group` AS `ch` ON ch.payment_id = pt.payment_tran_id
    LEFT JOIN `pay_group` AS `p` ON p.payment_id=pt.payment_id 
  ...
  WHERE 1 = 1 
    ...
    (SELECT 
      ck.sn FROM checkmoney ck 
        WHERE 1 = 1 ".$where_paytime.$where_invoicetime.$where_d_id.$where_sn.$where_invoice."
      AND ck.sn <> '0') 
  GROUP BY `ch`.`id`,`m`.`sn` )t group by t.payment_no_ref
            ";
...
/*--------------Total Use Deposit Amount------------------*/
          ...
          GROUP BY ch.id,
            ...
            $sql_result = "select t1.*,d.finance_group from (".$sql_total_pay_order.' UNION ALL '.$sql_checkmoney.' UNION ALL '.$sql_credit_note_tran.' UNION ALL '.$sql_pay_delivery_fee.' UNION ALL '.$sql_checkmoney_pay_banktransfer.' UNION ALL '.$sql_checkmoney_pay_servicecharge.' UNION ALL '.$sql_checkmoney_pay_service.' UNION ALL '.$sql_checkmoney_lack_of_money.' UNION ALL '.$sql_checkmoney_payment_surplus.' UNION ALL '.$sql_deposit_tran.")t1 LEFT JOIN `distributor` d ON d.id=t1.d_id where ".$userStorage->id."=".$userStorage->id." and type <> 6".$where_finaceGroup." order by t1.sn,t1.seq,t1.type desc,t1.pay_time";
...
////////////////////////////////////////////////
    ...
    public function _getDataForExcelDailyCashInView($data,$params)
    {
       ...
       // print_r($data);die;
        ...
        if($arr_d_id == 0){
          ...
          //   '2' as `TYPE`,
          //   '1' AS `company_id`,
          //    (ROUND(SUM(m.total)-(((m.`total`/m.`num`)*IFNULL(m.`spc_discount`,0))/100), 2) * - 1) AS `pay_money`,
          //    (ROUND(SUM(m.total)-(((m.`total`/m.`num`)*IFNULL(m.`spc_discount`,0))/100), 2) * - 1) AS `output`,
          //   '' AS `bank_transaction_code`,
          //   '' AS `content`,
          //   '' AS `note`,
          //   0 AS `balance`,
          ...
          //   `m`.`payment_type`,
          //   '' AS file_pay_slip, 
          //   '1' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`
          // FROM
          //    `market` AS `m` 
          // WHERE 1=1
          //  -- and m.canceled <>1
          ...
          //   FROM checkmoney ck 
          //   WHERE  
          //     1=1 ".$where_paytime.$where_invoicetime.$where_d_id.$where_sn."
          //     AND ck.sn <>'0') 
          //   GROUP BY `m`.`sn`";
          ...
          WHERE 1=1
           -- and m.canceled <>1
            ...
            `m`.`payment_type` ,
            `ch`.`file_pay_slip`,
            '2' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
          ...
          --  and m.canceled <>1
            ...
            m.payment_type,
            '' AS file_pay_slip,
            '3' as seq,'' as pay_text,m.`canceled`,m.`canceled_remark`
          FROM
            credit_note_tran AS ch 
            LEFT JOIN market AS m 
              ON m.sn = ch.sales_order 
          ...
          GROUP BY ch.id,
            ...
            `m`.`payment_type`,
            '' AS file_pay_slip,
            '4' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`
          FROM
             `market` AS `m` 
          WHERE 1=1
            AND m.delivery_fee >0
          --  and m.canceled <>1
            ...
            `m`.`payment_type`,
            ch.file_pay_slip,
            '5' as seq ,'' as pay_text,m.`canceled`,m.`canceled_remark`
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
          ...
          --  and m.canceled <>1
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '6' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
          ...
          --  and m.canceled <>1
            ...
            `m`.`payment_type`,
            ch.file_pay_slip, 
            '7' as seq,m.pay_text,m.`canceled`,m.`canceled_remark`
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
           ...
           -- and m.canceled <>1
            ...
            $sql_result = "select t1.*,d.finance_group from (".$sql_total_pay_order.' UNION ALL '.$sql_checkmoney.' UNION ALL '.$sql_credit_note_tran.' UNION ALL '.$sql_pay_delivery_fee.' UNION ALL '.$sql_checkmoney_pay_banktransfer.' UNION ALL '.$sql_checkmoney_pay_servicecharge.' UNION ALL '.$sql_checkmoney_pay_service.")t1 LEFT JOIN `distributor` d ON d.id=t1.d_id where ".$userStorage->id."=".$userStorage->id." and type <> 6".$where_finaceGroup." order by t1.sn,t1.seq,t1.type desc,t1.pay_time";
...
////////////////////////////////////////////////
    ...
    public function _getDataForExcelDailyCashInForBankCheck($data,$params)
    {
       ...
       // print_r($data);die;
        ...
        if($arr_d_id == 0){
            ...
            {
                $where_canceled=" AND (m.canceled = '".$params['cancel']."') ";
            }
...
/*
            ...
            '1' AS `company_id`,
            CASE
              WHEN m.d_id = 3691 && m.cat_id=11 && m.add_time <= '2017-01-04 00:00:00'
                THEN (ROUND(SUM(m.total)*1/100, 2))
              WHEN m.d_id = 3025 && m.add_time <= '2017-01-04 00:00:00'
                THEN (ROUND(SUM(m.total) * 1 / 100, 2))
              ELSE
                ifnull(m.total_spc_discount,0)
              END AS `pay_money`,
            CASE
              WHEN m.d_id = 3691 && m.cat_id=11 && m.add_time <= '2017-01-04 00:00:00'
                THEN (ROUND(SUM(m.total)*1/100, 2))
              WHEN m.d_id = 3025 && m.add_time <= '2017-01-04 00:00:00'
                THEN (ROUND(SUM(m.total) * 1 / 100, 2))
              ELSE
                ifnull(m.total_spc_discount*1.07,0)
              END AS `output`,
            '' AS `bank_transaction_code`,
            '' AS `content`,
            'Discount 1 %' AS `note`,
