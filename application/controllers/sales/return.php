...
$where = $QGoodCategory->getAdapter()->quoteInto('id = ?',12);
$this->view->good_categories = $QGoodCategory->fetchAll($where);
...
$select_group = $db->select()
        ...
        if ($result_group){
            ...
            $where[] = $QDistributor->getAdapter()->quoteInto('group_id in('.rtrim($group_id, ',').')', null);
        }

if($group_id !=''){
    $result_Distributor = $QDistributor->fetchAll($where, 'title');   
}else{
    $result_Distributor = $QDistributor->fetchAll();  
}   
//
 
//print_r($result_Distributor);  
$this->view->distributors = $result_Distributor;
