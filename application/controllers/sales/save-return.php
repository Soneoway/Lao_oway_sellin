...
if ($this->getRequest()->getMethod() == 'POST'){
    ...
    if (is_array($ids)) {
        ...
        try{
            ...
            //delete in IMEI return
            if (count($imei_list)==$count_input){
                $where_digital = $QDigitalReturn->getAdapter()->quoteInto('return_sn =?', $sn);
                $QDigitalReturn->delete($where_digital);
            }
            ...
            foreach ($imei_list as $imei){          
                ...
                */
                ...
                {
                    //digital
                    $where = $QDigital->getAdapter()->quoteInto('sn = ?' ,$imei);
                    $digital_check = $QDigital->fetchRow($where);

                    if(!isset($digital_check) || !$digital_check['distributor_id'] || !intval($digital_check['distributor_id']))
                    {
                        echo '<script>parent.palert("This IMEI / Digital '.$imei.' is not exitsted in our system.");</script>';
                    }

                    $where = $QDistributor->getAdapter()->quoteInto('id = ?', intval($digital_check['distributor_id']));
                    $imei_distributor = $QDistributor->fetchRow($where);
                    ...
                    if ($imei_distributor['id'] != $distributor_id && $imei_distributor['parent'] != $distributor_id) {
                        echo '<script>parent.palert("This IMEI / Digital '.$imei.' does not belong to this distributor.");</script>';
                        exit;
                    }
                }
            }
            ...
            if ($sn) {
                ...
                if(!empty($imei_check))
                {
                    ...
                    $old_imeireturns = $QDigitalReturn->fetchAll($where);
            ...
            foreach ($ids as $k=>$id){
                if (
                    ...
                    if ($cat_ids[$k] == $QGoodCategory->get_accessories_id()) 
                    {
                        $data = array(
                            'return_sn'     => $sn,
                            'cat_id'        => $cat_ids[$k],
            ...
            if ($this->getRequest()->getParam('sn')){
                ...
                $removed_imeireturns_list = array_diff($old_imeireturns_list, $imei_list);

                if(!empty($imei_check))
                {
                    if ($removed_imeireturns_list) {

                        //delete in IMEI return
                        $where = $QImeiReturn->getAdapter()->quoteInto('imei_sn IN (?)', $removed_imeireturns_list);
                        $QImeiReturn->delete($where);
                    }
                }else{
                    if ($removed_imeireturns_list) {
                        ...
                        //delete in IMEI return
                        $where = $QDigitalReturn->getAdapter()->quoteInto('sn IN (?)', $removed_imeireturns_list);
                        $QDigitalReturn->delete($where);
                    }
                }
            ...
            if ($new_imeireturns_list){
                ...
                if(!empty($imei_check))
                {
                    ...
                    foreach ($new_imeireturns_list as $imei) {
                        $imei = trim($imei);
                        $where = $QDigital->getAdapter()->quoteInto('sn = ?', $imei);
                        $imei_check = $QDigital->fetchRow($where);
                        $sales_sn = $imei_check['sales_sn'];        
