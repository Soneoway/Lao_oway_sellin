...
if ($sn) {
        ...
        OR (isset($warehouse_nvmm) AND $warehouse_nvmm))
    {
        ...
        if (isset($invoice[0]) and $invoice[0]) {
            if ($invoice[0]['invoice_type'] == 1) {
                /////có in hóa đơn
                $customer = array(
                    'name' => $invoice[0]['name'],
                    'company' => $invoice[0]['company'],
                    'tax_code' => $invoice[0]['tax_code'],
    ...
    //get nhan viem mua may
    /*
    $select_staff_nvmm = $db->select()
        ->from(array('a'=>'staff_order'),array('a.*',
            'staff_name_2'=>'CONCAT(b.firstname," ",b.lastname," (",b.ID_number,")")',
            'staff_name'=>'CONCAT(b.firstname," ",b.lastname," - ",b.ID_number)'
            ))
        ->joinLeft(array('b'=>HR_DB.'.staff'),'b.id = a.staff_id',array())
        ->where('sn = ?',$sn);
    $result_staff_nvmm = $db->fetchAll($select_staff_nvmm);
    $this->view->staff_nvmm = $result_staff_nvmm;

    if($result_staff_nvmm AND count($result_staff_nvmm)){

        $this->view->shipment_id = $result_staff_nvmm[0]['shipment_id'];
    }
    */
...
$where = array();
if($warehouse_type !=''){
   // $where[] = $QWarehouse->getAdapter()->quoteInto('warehouse_type IN ('.$warehouse_type.')', null);
   // $warehouses_cached1 = $QWarehouse->fetchAll($where, 'name');
    $warehouses_cached = $QWarehouse->get_cache();
    $this->view->warehouses = $warehouses_cached;
    
}else{
    $where[] = $QWarehouse->getAdapter()->quoteInto('warehouse_type =1', null);
    $warehouses_cached = $QWarehouse->fetchAll($where, 'name');
    $warehouse_arr = array();
    foreach ($warehouses_cached as $id=>$name){
        $warehouse_arr[$value['id']] = $value['name']; 
    }
...
$selectStaff = $db->select()
    ->from(array('a'=>HR_DB.'.staff'),array(
            'staff_id'     => 'a.id',
            'staff_name_2' => 'CONCAT(a.firstname," ",a.lastname," (", a.ID_number,")")',
            'staff_name'   => 'CONCAT(a.code ," - ",a.firstname," ",a.lastname," - ", a.ID_number," - ",e.name)',
        ))
    ->join(array('b'=>HR_DB.'.regional_market'),'b.id = a.regional_market',array())
    ->join(array('c'=>HR_DB.'.area'),'c.id = b.area_id',array())
    ->join(array('d'=>HR_DB.'.team'),'a.title = d.id',array())
    ->join(array('e'=>HR_DB.'.team'),'d.parent_id = e.id',array())
    ->where('a.off_date IS NULL')
    ->order('CONCAT(a.firstname," ",a.lastname) ASC')
;

$staffs_buy          = $db->fetchAll($selectStaff);
$this->view->objects = $staffs_buy;
*/

/*
$QShipment                  = new Application_Model_GoodShipment();
$this->view->shipment       = $QShipment->fetchAll();
*/
$this->view->id_userStorage = $userStorage->id;
$this->view->userStorage    = $userStorage;

//get service center from cs
...
// $wsParams = array();
// try{
//     $result_service = $client->call("getAllShowroom", $wsParams);
//     $this->view->service_center = $result_service;
// }catch (Exception $e){
//     echo 'get server center failure';
//     exit;
// }
