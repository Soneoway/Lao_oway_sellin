<?php
    ...
    if ($this->getRequest()->getMethod() == 'POST'){
            ...
            define('ROW_START', 2);
            define('GOOD_CODE', 0);
            define('GOOD_REPLACEMENT_CODE', 1);
            define('DETAIL', 2);
            define('CHANNEL', 3);
            define('COST_PRICE', 4);
            define('RETAIL_PRICE', 5);
            ...
            if (!$upload->isValid()) { // validate IF
                ...
                try {
                        ...
                        for ($i = ROW_START; $i <= $highestRow; $i++) 
                        {
                            ...
                            $good_replacement_code = trim($objWorksheet
                                ->getCellByColumnAndRow(GOOD_REPLACEMENT_CODE, $i)
                                ->getValue()); 
                            ...
                            $channel = trim($objWorksheet
                                ->getCellByColumnAndRow(CHANNEL, $i)
                                ->getValue()); 

                            $cost_price = trim($objWorksheet
                                ->getCellByColumnAndRow(COST_PRICE, $i)
                                ->getValue()); 

                            $retail_price = trim($objWorksheet
                                ->getCellByColumnAndRow(RETAIL_PRICE, $i)
                                ->getValue()); 
                            ...
                            if($good_code !=""){
                                ...
                                $params = array(
                                    ...
                                    'good_replacement_code'     => trim($good_replacement_code),
                                    'detail'                    => trim($detail),
                                    'channel'                   => trim($channel),
                                    'price'                     => trim($cost_price),
                        ...
                        if (is_array($error_list) && count($error_list) > 0) 
                        {
                            ...
                            $objWorksheet_out->setCellValueByColumnAndRow(GOOD_REPLACEMENT_CODE, 1, 'Good Replacement Code');
                            $objWorksheet_out->setCellValueByColumnAndRow(DETAIL, 1, 'Detail');
                            $objWorksheet_out->setCellValueByColumnAndRow(CHANNEL, 1, 'Channel');
                            $objWorksheet_out->setCellValueByColumnAndRow(PRICE, 1, 'Price');
                            $objWorksheet_out->setCellValueByColumnAndRow(RETAIL_PRICE + 1, 1, 'Retail Price');

                            $i = 2;
                            foreach ($error_list as $key => $row)
                            {
                                $objWorksheet_out->setCellValueByColumnAndRow(GOOD_CODE, $i, $row['good_code']);
                                $objWorksheet_out->setCellValueByColumnAndRow(GOOD_REPLACEMENT_CODE, $i, $row['GOOD_REPLACEMENT_CODE']);
                                $objWorksheet_out->setCellValueByColumnAndRow(DETAIL, $i, $row['detail']);
                                $objWorksheet_out->setCellValueByColumnAndRow(CHANNEL, $i, $row['channel']);
                                $objWorksheet_out->setCellValueByColumnAndRow(PRICE, $i, $row['price']);
                                $objWorksheet_out->setCellValueByColumnAndRow(RETAIL_PRICE +1, $i, $row['retail_price']);
                                $i++;
                            }
