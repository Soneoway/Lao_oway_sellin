...
if ($this->getRequest()->getMethod() == 'GET'){
	...
	if($act=='new'){
		...
		if($create_lot_number==true){
            ...
            echo '<script>parent.location.href="/finance/cp-auto-check-imei?act=edit&lot_sn='.$lot_sn.'&lot_number='.$lot_number.'"</script>';
			exit;
        }else{
            $db->rollback();
		...
		if ($lot_sn)
		{
		    ...
		    if($lot_sn !=''){
				...
				//print_r($group_by_imei);die;
		        $imei_check=null;
			    $imei_check = $QCPAutoCheckImei->checkPriceProtectionImeiAutoCheckListAction($lot_sn,"finance",$group_by_imei);
			    //print_r($imei_check);
