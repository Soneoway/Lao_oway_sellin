    ...
    if (!$upload->isValid()){ // validate IF
        ...
        try {
            ...
            for ($i = STORE_CODE_LIST_ROW_START;
                ...
                //load so hoa don tu dong vao don hang theo tung kho
                ...
                $invoice_number_data = array_filter(array(
                   'invoice_sign'   => $invoice_prefix['id'],
                ...
                if(is_array($invoice_number_data))
                {

                    $invoice_number_data = serialize($invoice_number_data);
                    $dataInvoice = array('invoice_number' => $invoice_number,
                        'date'         => $currentTime ,
                        'invoice_sign' => $invoice_prefix['id'],
                        'sn'           => $sn,
                    );
                    //them so invoice vao he thong
                    $id = $QInvoiceNumber->insert($dataInvoice);
                }
                ...
                $params = array(
                    ...
                    'invoice_number_data' => $invoice_number_data
                );
