...
if(!isset($distributor['group_id']) || !$distributor['group_id']){
    echo 'Please Set Distributor Type Group On Distributor Details Page';
    die;
}

$QDG = new Application_Model_DistributorGroup();
$dis_group_type = $QDG->find($distributor['group_id']);
$dis_group_type = $dis_group_type->current();

$market['group_type_id'] = '';
if(isset($dis_group_type['group_type_id']) && $dis_group_type['group_type_id']){
    $market['group_type_id'] = $dis_group_type['group_type_id'];
}

$market['group_name'] = '';
if(isset($dis_group_type['group_name']) && $dis_group_type['group_name']){
    $market['group_name'] = $dis_group_type['group_name'];
}

$market['job_sn'] = '';
//Distributor type group 9 = service
if($market['group_type_id'] == 9){
    $QJ = new Application_Model_JobNumber();
    $service_job = $QJ->getJobNumber('',$market['sn']);
    $market['job_sn'] = $service_job['job_sn'];
}
