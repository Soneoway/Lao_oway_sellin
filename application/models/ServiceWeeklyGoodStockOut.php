<?php
class Application_Model_ServiceWeeklyGoodStockOut extends Zend_Db_Table_Abstract{
	protected $_name = 'service_weekly_good_stock_out';

	function ServiceWeeklyProductStockOutListfetchPagination($page, $limit, &$total, $params){
        $db = Zend_Registry::get('db');
        $userStorage = Zend_Auth::getInstance()->getStorage()->read();

        //print_r($params);die;
        $select = $db->select();
        $select->from(array('p' => $this->_name),array(new Zend_Db_Expr('SQL_CALC_FOUND_ROWS p.id'),'p.data_date','p.good_code','p.price_cost_usd','p.price_cost_bath','p.inside_num','p.outside_num','p.in_out_side_num','p.inside_total_price','p.outside_total_price','p.in_out_side_total_price','p.created_date','p.status','p.week_of_year'));

        $select->joinleft(array('s'=>'staff'),'p.created_by=s.id',array("created_by"=>"concat(s.firstname,' ',s.lastname)",'s.email'));
