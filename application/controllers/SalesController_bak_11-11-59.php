...
/*
           ...
           */
            if($item['canceled']==1){
                ...
                if ($item['d_id']== '3691' || $item['d_id']== '3025') {
                    $add_time = '2016-11-01 00:00:00';
                    $create_at = $item['create_at'];
                    if(strtotime($create_at) > strtotime($add_time)){
                        $show="new";
                        if ($item['cat_id']==11) {
                           $total_discount = 1;
    ...
    {   
        ...
        if ($sn) {
           ...
           // $remain_balance                  = ($company_id == 1) ? $distributor_balance_row['balance'] : $distributor_balance_row['balance_smartmobile'];
            ...
            $QBank = new Application_Model_Bank();
            $where = $QBank->getAdapter()->quoteInto('type_payment = ?','TR');
