<?php
class Application_Model_ServiceWeeklyGoodStockIn extends Zend_Db_Table_Abstract{
	protected $_name = 'service_weekly_good_stock_in';

	function ServiceWeeklyProductStockInListfetchPagination($page, $limit, &$total, $params){
        $db = Zend_Registry::get('db');
        $userStorage = Zend_Auth::getInstance()->getStorage()->read();

        //print_r($params);die;
        $select = $db->select();
        $select->from(array('p' => $this->_name),array(new Zend_Db_Expr('SQL_CALC_FOUND_ROWS p.id'),'p.data_date','p.good_code','p.num','p.price_cost_usd','p.price_cost_bath','p.price_cost_total_bath','p.created_date','p.status','p.week_of_year'));
