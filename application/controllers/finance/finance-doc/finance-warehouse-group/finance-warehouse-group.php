...
$params = array(
	'dis_id'					=> $dis_id,
	'finance_warehouse_group'	=> $finance_warehouse_group,
);
...
$finance_warehouse_group = $QFinanceWarehouseGroup->fetchPagination($page, $limit, $total, $params);

if(isset($export) && $export) {

	$data = $QFinanceWarehouseGroup->fetchPagination($page, null, $total, $params);

	$this->_exportFinanceWarehouseGroup($data);
	
}

$this->view->distributor = $distributor_arr;
$this->view->finance_warehouse_group = $finance_warehouse_group;
...
$this->view->url    = HOST.'finance/finance-warehouse-group/'.( $params ? '?'.http_build_query($params).'&' : '?' );
...
$this->_helper->viewRenderer->setRender('finance-doc/finance-warehouse-group');
