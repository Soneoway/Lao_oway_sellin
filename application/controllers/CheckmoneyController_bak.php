...
class CheckmoneyController extends My_Controller_Action{
    ...
    public function listAction(){
       ...
       // $to_time               = $this->getRequest()->getParam('to_time');
        ...
        if((isset($_GET['SearchBox']) ? true: false)==true)
        {
    ...
    */
        ...
        if($arr_d_id == 0){
            ...
            `m`.`payment_type` 
            ,'1' as seq
            ,'Total Order Amount' as transaction_type
          FROM
             `market` AS `m` 
          WHERE 1=1
            and m.canceled <>1
            ...
            `m`.`payment_type` 
            ,'2' as seq
            ,'Tranfer Money Amount' as transaction_type
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            ,m.payment_type
            ,'3' as seq
            ,'Total Use Credit Note Amount' as transaction_type
          FROM
          ...
          GROUP BY ch.id,
            ...
            `m`.`payment_type`
            ,'4' as seq 
            ,'Total Delivery Fee Amount' as transaction_type
          FROM
             `market` AS `m` 
          WHERE 1=1
            ...
            `m`.`payment_type`
            ,'5' as seq 
            ,'Total pay banktransfer Amount' as transaction_type
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type` 
            ,'6' as seq
            ,'Total pay_servicecharge Amount' as transaction_type
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type` 
            ,'7' as seq
            ,'Total pay service Amount' as transaction_type
            FROM
              `checkmoney` AS `ch` 
              LEFT JOIN `bank` AS `b` 
                ON b.id = ch.bank 
            ...
            `m`.`payment_type` 
            ,'8' as seq
            ,'Total Discount 1 %' AS transaction_type
          FROM
             `market` AS `m` 
          WHERE 1=1
            and m.canceled <>1
