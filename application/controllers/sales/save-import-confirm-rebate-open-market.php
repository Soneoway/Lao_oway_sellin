            ...
            if (!$upload->isValid()) { // validate IF
                ...
                try {
                        ...
                        $imei_sn = trim($objWorksheet
                            ->getCellByColumnAndRow(IMEI_SN, $i)
                            ->getFormattedValue()); 
                        
                        $sim_serials = trim($objWorksheet
                            ->getCellByColumnAndRow(SIM_SERIAL, $i)
                            ->getValue()); 

                        $sim_serial = ltrim ($sim_serials , "'");

                        $true_code = trim($objWorksheet
                            ->getCellByColumnAndRow(TRUE_CODE, $i)
                            ->getValue()); 
                        ...
                        $params = array(
                            'imei_sn'                           => $imei_sn,
                            'sim_serial'                        => $sim_serials,
                            'true_code'                         => $true_code,
                            'confirm_rebate_by'                 => $userStorage->id,
                            'confirm_rebate_date'               => $date,
                        );

                        /*$data = array(
                            'confirm_rebate_by'                 => null,
                            'confirm_rebate_date'               => null,
                        );*/
                        ...
                        //print_r($sim_serials);
                        $where = array();
                        $where[] = $QPackedSim->getAdapter()->quoteInto('imei_sn = ?', $imei_sn);
                        $where[] = $QPackedSim->getAdapter()->quoteInto('simcard = ?', $sim_serial);
                        //$where[] = $QPackedSim->getAdapter()->quoteInto('true_code = ?', $true_code);
                        //print_r($where);die;
                        $result=$QPackedSim->update($data_update, $where);
                    ...
                    //print_r($success_list);die;
                        ...
                        if (is_array($error_list) && count($error_list) > 0) 
                        {
                            ...
                            $objWorksheet_out->setCellValueByColumnAndRow(IMEI_SN, 1, 'Imei SN');
                            $objWorksheet_out->setCellValueByColumnAndRow(SIM_SERIAL, 1, 'SIM Serial');
                            $objWorksheet_out->setCellValueByColumnAndRow(TRUE_CODE, 1, 'True Code');
                            $objWorksheet_out->setCellValueByColumnAndRow(IMEI_SN + 1, 1, 'Messages');


                            // các dòng lỗi
                            $i = 2;
                            foreach ($error_list as $key => $row) {
                                $objWorksheet_out->setCellValueByColumnAndRow(IMEI_SN, $i, $row['imei_sn']);
                                $objWorksheet_out->setCellValueByColumnAndRow(SIM_SERIAL, $i, $row['sim_serial']);
                                $objWorksheet_out->setCellValueByColumnAndRow(TRUE_CODE, $i, $row['true_code']);
                                $objWorksheet_out->setCellValueByColumnAndRow(IMEI_SN +1, $i, $row['message']);
                                $i++;
                            }
                            //
                            switch ($extension) {
