...
class Application_Model_Checkmoney extends Zend_Db_Table_Abstract
{
	...
	public function fetchPaginationHistory($page, $limit, &$total, $params){
        ...
        if(isset($params['is_credit']) AND $params['is_credit'] = 'CREDIT'){
            ...
            $select->where($db->quoteInto('ch.credit_tr_id is null',1));
        }
    ...
    public function _getMoneyIn($params,&$from = '',&$to =''){
        ...
        if(isset($params['from_invoice_time']) AND $params['from_invoice_time'] || isset($params['to_invoice_time']) AND $params['to_invoice_time']){
            ...
            ->where('d.del = 0 OR d.del is null',1)
            ->group('m.sn','ch.d_id');
...
tb.sn,tb.TYPE,tb.payment_no,tb.lack_of_money,tb.payment_surplus,tb.company_id,IF(tb.pay_money>0,tb.pay_money*-1,tb.pay_money) AS pay_money,IF(tb.pay_money>0,tb.pay_money*-1,tb.pay_money) AS output,
tb.bank_transaction_code,tb.content,tb.note,tb.balance,tb.d_id,tb.pay_time,tb.creditnote_sn,tb.deposit_sn,
tb.bank_name,tb.credit_card,tb.invoice_number,tb.sn_ref,tb.payment_type,tb.file_pay_slip,tb.seq,tb.pay_text,
tb.canceled,tb.canceled_remark,tb.sales_order_ref,tb.payment_no_ref,tb.bs_campaign,tb.phone_number_sn
  from
  (SELECT 
            `m`.`sn`,
            '2' as `TYPE`,m.payment_no,0 as lack_of_money,0 as payment_surplus,
            '1' AS `company_id`,
             (SELECT IFNULL((SUM(cnt.use_discount)-ch.pay_money),ch.pay_money) FROM checkmoney ch LEFT JOIN credit_note_tran cnt ON CAST(cnt.sales_order AS CHAR) = CAST(ch.sn AS CHAR)  WHERE ch.sn = m.sn AND ch.type = 2) AS `pay_money`,
             0 AS `output`,
          ...
          GROUP BY pt.id
            ...
            WHERE  
              1=1 ".$where_paytime.$where_invoicetime.$where_d_id.$where_sn.$where_invoice."
              AND ck.sn <>'0')
            GROUP BY m.payment_no,bg.`balance`,bg.file_pay_slip,bg.pay_date
            ";        
