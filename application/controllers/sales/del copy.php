    ...
    if ($sales) {
        ...
        */
        ...
        if (!isset($sales['user_id'])) {
            $flashMessenger->setNamespace('error')->addMessage('Invalid user');
            $this->_redirect($back_url);
        } else if ($market['user_id'] != $userStorage->id && !My_Staff_Group::inGroup($userStorage->group_id, ADMINISTRATOR_ID))
        {

            if ($market['user_id'] != $userStorage->id && !My_Staff_Group::inGroup($userStorage->group_id, SALES_ADMIN_ACCESSORIES))
            {
                if ($market['user_id'] != $userStorage->id && !My_Staff_Group::inGroup($userStorage->group_id, SALES_ADMIN))
                {
