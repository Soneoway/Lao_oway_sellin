...
$refer = My_Url::refer(HOST.'delivery/order-control');
...
try {
    ...
    if ($order['type'] != My_Delivery_Type::Outside) throw new Exception("Only Ouside carrier need update information", 3);
    if ($order['status'] != My_Delivery_Order_Status::Waiting) throw new Exception("Order not in waiting status", 4);

    if ( isset($order['hub']) && !My_Delivery_Role::isAllow($userStorage->group_id, $userStorage->id, $order['hub'], My_Delivery_Role::Hub) )
        throw new Exception("You have no permission", 5);
