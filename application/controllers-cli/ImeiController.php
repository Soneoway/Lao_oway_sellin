...
//        error_reporting(~E_ALL);
    ...
    public function bydateAction() {
        date_default_timezone_set('Asia/Shanghai');
        $datetime = $this->getRequest()->getParam('datetime',3);
    ...
    private function syncActivatedTime()
    {
        ...
        $sql = "UPDATE imei, imei_activation
                SET imei.activated_date = ADDTIME(imei_activation.activated_at,'05:00:00') 
                WHERE imei_activation.imei_sn=imei.imei_sn and imei.activated_date IS NULL";
        $this->sqlquery(DB_HOST,DB_USER,DB_PASS,DB_NAME,$sql);
    }

    //Truncate Imei Activation
    function clearImeiActivationAction(){
        date_default_timezone_set('Asia/Shanghai');
        $sql = "truncate table imei_activation;";
        $this->sqlquery(DB_HOST,DB_USER,DB_PASS,DB_NAME,$sql);
    }
    //Truncate Log Market
    function clearLogMarketAction(){
        date_default_timezone_set('Asia/Shanghai');
        $sql = "truncate table log_market;";
        $this->sqlquery(DB_HOST,DB_USER,DB_PASS,DB_NAME,$sql);

        $sql = "truncate table checkmoney_log;";
        $this->sqlquery(DB_HOST,DB_USER,DB_PASS,DB_NAME,$sql);
    }
    ...
    private function read_one_day($date)
    {
        ...
        $server = 'http://imei.console.vn/ws-imei';
        $page = 1;
        // $limit = LIMITATION;
