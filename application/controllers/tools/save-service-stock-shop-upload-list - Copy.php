    ...
    if ($this->getRequest()->getMethod() == 'POST'){
            ...
            if (!$upload->isValid()) { // validate IF
                ...
                try {
                        ...
                        //print_r($data_date);die;
                        //$date = explode(" ", $data_date);

                        $create_date = date('Y-m-d H:i:s');
                        $date = DateTime::createFromFormat('d/m/Y', $data_date);
                        //$date = DateTime::createFromFormat('MM yyyy', $data_date);

                        //echo $date[0];die;
                        //$monthStr = getMonthNumber($date[0]);

                        $data_date1 = $date->format('Y-m-d');

                        //echo $monthStr;die;

                        $where = $QServiceStockShopList->getAdapter()->quoteInto('data_date = ?', $data_date1);
                        ...
                        for ($c = 1; $c < $columnIndex-1; $c++) 
                        {
                            $warehouse_name = trim($objWorksheet
                                ->getCellByColumnAndRow($c, 14)
                                ->getValue());
