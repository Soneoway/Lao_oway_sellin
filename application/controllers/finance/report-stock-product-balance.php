...
$this->_helper->viewRenderer->setRender('report-stock-product-balance');
...
if ( isset($export) && $export == 1 ) {
    ...
    //print_r($data);die;
    $this->_exportExcel_stock_product_balance($data, $params);
}else {  //show list
    $data = $QDailyStockBalance->fetchPagination($page, $limit, $total, $params);
}
...
$this->view->url    = HOST.'finance/report-stock-product-balance/'.( $params ? '?'.http_build_query($params).'&' : '?' );
