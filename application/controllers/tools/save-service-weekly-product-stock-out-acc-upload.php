<?php
    ...
    if ($this->getRequest()->getMethod() == 'POST'){
            ...
            if (!$upload->isValid()) { // validate IF
                ...
                try {
                        $QServiceGoodStockOutAcc = new Application_Model_ServiceWeeklyGoodStockOutAcc();
                        ...
                        $week = $QServiceGoodStockOutAcc->getWeekOfYear($data_date);
                        $week_of_year=$week['week_of_year'];
                        $where = array();
                        $where[] = $QServiceGoodStockOutAcc->getAdapter()->quoteInto('YEAR(data_date) = YEAR("'.$data_date.'")', null);
                        $where[] = $QServiceGoodStockOutAcc->getAdapter()->quoteInto('WEEK(data_date) = WEEK("'.$data_date.'")', null);
                        ...
                        $where = array();
                        $where[] = $QServiceGoodStockOutAcc->getAdapter()->quoteInto('data_date >= ?', $stock_date."-01");
                        $where[] = $QServiceGoodStockOutAcc->getAdapter()->quoteInto('data_date <= ?', $stock_date."-31");*/

                        //print_r($where);die;
                        $code_check = $QServiceGoodStockOutAcc->fetchRow($where);
                        //print_r($where);die;
                        if ($code_check) {
                            $result_del=$QServiceGoodStockOutAcc->delete($where);
                        }
                        //echo $stock_date;die;
                        for ($i = ROW_START; $i <= $highestRow; $i++) 
                        {
                            ...
                            $total_price = trim($objWorksheet
                                ->getCellByColumnAndRow(total_price, $i)
                                ->getValue()); 
                            ...
                            if($good_code !=""){
                                ...
                                //print_r($params);die;
                                $result=$QServiceGoodStockOutAcc->insert($params); 
                                //
                        ...
                        if (is_array($error_list) && count($error_list) > 0) 
                        {
                            ...
                            $objWorksheet_out->setCellValueByColumnAndRow(total_price, 1, 'total_price');
                            ...
                            foreach ($error_list as $key => $row)
                            {
                                ...
                                $objWorksheet_out->setCellValueByColumnAndRow(total_price, $i, $row['total_price']);
                                $i++;
                            }
