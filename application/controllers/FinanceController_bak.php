...
class FinanceController extends My_Controller_Action
{
            ...
            if ($this->getRequest()->getMethod() == 'POST') {
                ...
                if ($payment) {
                    ...
                    try {
                        ...
                        if($create_cn==1){
                        //Tanong
                            $this->get_credit_note_sn($db,$distributor_id,$userStorage->id,$sn);
                        }
                        //todo log
                        $ip = $this->getRequest()->getServer('REMOTE_ADDR');
    ...
    public function get_credit_note_sn($db,$distributor_id,$user_id,$sn)
    {
    try {
            $flashMessenger = $this->_helper->flashMessenger;

            $db->query("CALL gen_credit_note_sn_new_return('".$distributor_id."','".$user_id."',".$sn.")");

        }catch (exception $e) {
            $flashMessenger->setNamespace('error')->addMessage('Cannot Create Create Note No, please try again!');
        }
    }
    ...
    public function getReward_CreateNoteNo_Ref($db,$distributor_id,$user_id,$sn)
    {
        ...
        $creditnote_sn="";
        try {
            $stmt = $db->prepare("CALL gen_running_no_ref('CN',".$sn.")");
            $stmt->execute();
            $result = $stmt->fetch();
            $creditnote_sn = $result['running_no'];
        }catch (exception $e) {
    ...
    function saveOppoClupRewardConfirmAction()
    {
        ...
        if ($this->getRequest()->getMethod() == 'POST') {
            ...
            define('MASS_BVG_LIST_COL_creditnote_price_confirm', 9);
