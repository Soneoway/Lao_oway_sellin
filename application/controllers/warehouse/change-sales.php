...
if($this->getRequest()->isPost()) {
   ...
   // $pay            = $this->getRequest()->getParam('pay',null); // pay money or Not
    ...
    // check dupilcate product
    $tempCheckDupPro = [];

    $countCheckDupPro = 0;
    foreach ($good_ids as $key_goods) {

        if(!$tempCheckDupPro){
            array_push($tempCheckDupPro, ['good_id' => $key_goods,'color_id' => $good_colors[$countCheckDupPro]]);
        }else{
            foreach ($tempCheckDupPro as $key_temp) {
                if($key_temp['good_id'] == $key_goods && $key_temp['color_id'] == $good_colors[$countCheckDupPro]){
                    echo '<script>window.parent.document.getElementById("iframe").style.display = \'block\';</script>';
                    echo '<script>window.parent.document.getElementById("iframe").height = \'40px\';</script>';
                    echo '<div class="alert alert-error">Failed - '.$goods_cached[$key_goods].' | '.$good_colors_cached[$good_colors[$countCheckDupPro]].' is dupilcate!</div>';
                    exit;
                }
            }
            array_push($tempCheckDupPro, ['good_id' => $key_goods,'color_id' => $good_colors[$countCheckDupPro]]);
